...
# ========== ГЕНЕРАЦИЯ API-КЛЮЧЕЙ (Аналог Devious/WTF) ==========
api_keys = {}  # {api_key: {"balance": int, "used": int}}

@app.route('/funcaptcha/createKey', methods=['POST'])
def create_key():
    """Создает API-ключ формата axora-XXXXXXXXXXXX"""
    try:
        data = request.get_json() or {}
        initial_balance = data.get("balance", 100)
        
        # Генерируем ключ
        key = f"axora-{secrets.token_hex(8)}"
        api_keys[key] = {
...
@app.route('/funcaptcha/getBalance', methods=['POST'])
def get_balance():
    """Получает баланс по API-ключу"""
    try:
        data = request.get_json()
        key = data.get('apiKey')
        
        if not key:
            return {"success": False, "error": "apiKey required"}
        ...
        return {
            "success": True,
            "balance": api_keys[key]["balance"],
            "used": api_keys[key]["used"],
            "total": api_keys[key]["balance"] + api_keys[key]["used"]
        }
    except Exception as e:
        return {"success": False, "error": str(e)}

@app.route('/funcaptcha/listKeys', methods=['GET'])
def list_keys():
    """Список всех ключей (для администрирования)"""
    return {
        "success": True,
...
# ========== МОДИФИЦИРОВАННЫЙ createTask С ПРОВЕРКОЙ КЛЮЧА ==========
...
def createTask():
    try:
        ...
        # Проверяем баланс
        if api_keys[api_key]["balance"] <= 0:
            return {"success": False, "error": "Insufficient balance. Please refill."}
        
        # Добавляем пресет
        data = {**data, **preset.get()[data["preset"]]}
        del data['preset']
        
        # Убираем apiKey из данных задачи
        api_key_value = data.pop('apiKey', None)
        ...
        self.pool[task_id] = (data, {
            ...
            "task_id": task_id,
            "result": None,
            "api_key": api_key_value  # сохраняем ключ для списания
        })
...
# ========== МОДИФИЦИРОВАННЫЙ solve С СПИСАНИЕМ БАЛАНСА ==========
def solve_with_balance(self, task_id):
    j, task_data = self.pool[task_id]
    ...
    # Если успешно — списываем баланс
    if result.get("success") and api_key and api_key in api_keys:
        api_keys[api_key]["balance"] -= 1
...
# ========== ПЕРЕОПРЕДЕЛЯЕМ МЕТОД SOLVE В API ==========
api.solve = solve_with_balance
